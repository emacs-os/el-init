...
;;; Overrides (see supervisor-overrides.el)
;;
;; The overrides persistence subsystem, effective state getters, and
;; policy mutators are extracted to supervisor-overrides.el.
;; All defvar definitions referenced by supervisor-overrides are
...
;;; Logging (see supervisor-log.el)
;;
;; The logging subsystem is extracted to supervisor-log.el.
;; All defcustom/defvar definitions referenced by supervisor-log are
;; above this point, so the require is safe here.
...
;;; Libexec helpers (see supervisor-libexec.el)
;;
;; Build target enumeration, compiler selection, and build invocation
;; are extracted to supervisor-libexec.el.
...
;;; Sandbox profiles (see supervisor-sandbox.el)
;;
