;;; elinit-test-validation.el --- Entry validation and hardening tests for elinit.el -*- lexical-binding: t -*-
...
;; Entry validation and hardening ERT tests for elinit.el.
...
(ert-deftest elinit-test-validate-string-entry ()
  "String entries are always valid."
  (should (null (elinit--validate-entry "nm-applet")))
  (should (null (elinit--validate-entry "/usr/bin/foo"))))

(ert-deftest elinit-test-validate-valid-simple ()
  "Valid simple entry passes validation."
  (should (null (elinit--validate-entry
                 '("nm-applet" :type simple :restart t)))))

(ert-deftest elinit-test-validate-valid-oneshot ()
...
(ert-deftest elinit-test-validate-rejects-legacy-async ()
...
(ert-deftest elinit-test-validate-invalid-oneshot-timeout ()
...
(ert-deftest elinit-test-validate-stage-must-be-symbol ()
...
(ert-deftest elinit-test-validate-unit-file-odd-plist ()
  "Odd-length plist is rejected by unit-file validation."
  (let ((result (elinit--validate-unit-file-plist
                 '(:id "svc" :command "cmd" :enabled) "test.el" 1)))
    (should (stringp result))
    (should (string-match-p "odd number of elements" result))))

(ert-deftest elinit-test-validate-unit-file-dotted-plist ()
...
(ert-deftest elinit-test-validate-entry-whitespace-string-command ()
  "Whitespace-only string command is rejected by entry validation."
  (let ((result (elinit--validate-entry "   ")))
    (should (stringp result))
    (should (string-match-p "empty or whitespace-only" result))))

(ert-deftest elinit-test-validate-entry-empty-list-command ()
  "Empty command in list entry is rejected by entry validation."
  (let ((result (elinit--validate-entry '("" :id "svc"))))
    (should (stringp result))
    (should (string-match-p "empty or whitespace-only" result))))

(ert-deftest elinit-test-validate-unit-file-empty-command ()
...
(ert-deftest elinit-test-validate-unit-file-whitespace-command ()
...
(ert-deftest elinit-test-validate-entry-id-with-control-char ()
  "ID containing control characters is rejected by entry validation."
  (let ((result (elinit--validate-entry '("cmd" :id "foo\nbar"))))
    (should (stringp result))
    (should (string-match-p "invalid characters" result))))

(ert-deftest elinit-test-validate-entry-id-valid-chars ()
  "ID with valid characters passes entry validation."
  (let ((result (elinit--validate-entry '("cmd" :id "my-svc_2.0:main@host"))))
    (should-not result)))

(ert-deftest elinit-test-validate-unit-file-id-invalid-chars ()
...
(ert-deftest elinit-test-validate-boolean-disabled-string ()
  "String value for :disabled is rejected."
  (let ((result (elinit--validate-entry '("cmd" :disabled "no"))))
    (should (stringp result))
    (should (string-match-p ":disabled must be t or nil" result))))

(ert-deftest elinit-test-validate-boolean-logging-number ()
  "Numeric value for :logging is rejected."
  (let ((result (elinit--validate-entry '("cmd" :logging 1))))
    (should (stringp result))
    (should (string-match-p ":logging must be t or nil" result))))

(ert-deftest elinit-test-validate-boolean-no-restart-string ()
  "String value for :no-restart is rejected."
  (let ((result (elinit--validate-entry '("cmd" :no-restart "no"))))
    (should (stringp result))
    (should (string-match-p ":no-restart must be t or nil" result))))

(ert-deftest elinit-test-validate-boolean-oneshot-blocking-string ()
...
(ert-deftest elinit-test-validate-boolean-oneshot-async-string ()
...
(ert-deftest elinit-test-validate-tags-number ()
  "Numeric :tags value is rejected."
  (let ((result (elinit--validate-entry '("cmd" :tags 42))))
    (should (stringp result))
    (should (string-match-p ":tags must be a symbol, string, or list" result))))

(ert-deftest elinit-test-validate-tags-valid-symbol-list ()
  "List of symbols is valid for :tags."
  (should-not (elinit--validate-entry '("cmd" :tags (web api)))))

(ert-deftest elinit-test-validate-tags-valid-string-list ()
  "List of strings is valid for :tags."
  (should-not (elinit--validate-entry '("cmd" :tags ("web" "api")))))

(ert-deftest elinit-test-validate-tags-empty-string ()
  "Empty string in :tags is rejected."
  (let ((result (elinit--validate-entry '("cmd" :tags ""))))
    (should (stringp result))
    (should (string-match-p "empty strings" result))))

(ert-deftest elinit-test-validate-tags-nil-element ()
  "Nil element in :tags list is rejected."
  (let ((result (elinit--validate-entry '("cmd" :tags (web nil)))))
    (should (stringp result))
    (should (string-match-p "nil values" result))))

;;; V6: Dependency and timeout hardening tests

(ert-deftest elinit-test-validate-oneshot-timeout-negative ()
...
(ert-deftest elinit-test-validate-oneshot-timeout-zero ()
...
(ert-deftest elinit-test-validate-oneshot-timeout-valid ()
  "Positive :oneshot-timeout passes validation."
  (should-not (elinit--validate-entry
               '("cmd" :type oneshot :oneshot-timeout 5))))

(ert-deftest elinit-test-validate-after-empty-string ()
  "Empty string in :after is rejected."
  (let ((result (elinit--validate-entry '("cmd" :after ""))))
    (should (stringp result))
    (should (string-match-p ":after must not contain empty" result))))

(ert-deftest elinit-test-validate-requires-empty-string ()
  "Empty string in :requires is rejected."
  (let ((result (elinit--validate-entry '("cmd" :requires ""))))
    (should (stringp result))
    (should (string-match-p ":requires must not contain empty" result))))

(ert-deftest elinit-test-validate-before-empty-string ()
  "Empty string in :before is rejected."
  (let ((result (elinit--validate-entry '("cmd" :before ""))))
    (should (stringp result))
    (should (string-match-p ":before must not contain empty" result))))

(ert-deftest elinit-test-validate-wants-empty-string ()
  "Empty string in :wants is rejected."
  (let ((result (elinit--validate-entry '("cmd" :wants ""))))
    (should (stringp result))
...
(ert-deftest elinit-test-validate-requires-self-dependency ()
  "Self-reference in :requires is rejected."
  (let ((result (elinit--validate-entry '("cmd" :id "a" :requires "a"))))
    (should (stringp result))
    (should (string-match-p ":requires must not reference the entry's own ID" result))))

(ert-deftest elinit-test-validate-before-self-dependency ()
  "Self-reference in :before is rejected."
  (let ((result (elinit--validate-entry '("cmd" :id "a" :before "a"))))
    (should (stringp result))
    (should (string-match-p ":before must not reference the entry's own ID" result))))

(ert-deftest elinit-test-validate-wants-self-dependency ()
  "Self-reference in :wants is rejected."
  (let ((result (elinit--validate-entry '("cmd" :id "a" :wants "a"))))
    (should (stringp result))
...
(ert-deftest elinit-test-validate-restart-sec-with-restart-no ()
...
(ert-deftest elinit-test-validate-restart-sec-with-restart-nil ()
...
(ert-deftest elinit-test-validate-restart-sec-with-restart-always ()
  "Restart-sec with :restart always is valid."
  (should-not (elinit--validate-entry
               '("cmd" :restart always :restart-sec 5))))

(ert-deftest elinit-test-validate-restart-sec-nil-with-no-restart ()
...
(ert-deftest elinit-test-validate-requires-self-dependency-derived-id ()
  "Self-reference in :requires detected via derived command ID."
  (let ((result (elinit--validate-entry '("svc" :requires "svc"))))
    (should (stringp result))
    (should (string-match-p ":requires must not reference the entry's own ID" result))))

(ert-deftest elinit-test-validate-before-self-dependency-derived-id ()
  "Self-reference in :before detected via derived command ID."
  (let ((result (elinit--validate-entry '("svc" :before "svc"))))
    (should (stringp result))
    (should (string-match-p ":before must not reference the entry's own ID" result))))

(ert-deftest elinit-test-validate-wants-self-dependency-derived-id ()
  "Self-reference in :wants detected via derived command ID."
  (let ((result (elinit--validate-entry '("svc" :wants "svc"))))
    (should (stringp result))
    (should (string-match-p ":wants must not reference the entry's own ID" result))))
...
(ert-deftest elinit-test-validate-environment-key-with-space ()
...
(ert-deftest elinit-test-validate-environment-key-with-equals ()
...
(ert-deftest elinit-test-validate-environment-valid-key ()
...
(ert-deftest elinit-test-validate-exec-stop-list-with-empty ()
...
(ert-deftest elinit-test-validate-exec-reload-empty-string ()
  "Empty :exec-reload string is rejected."
  (let ((result (elinit--validate-entry '("cmd" :exec-reload ""))))
    (should (stringp result))
...
(ert-deftest elinit-test-validate-success-exit-status-over-255 ()
  "Exit code over 255 in :success-exit-status is rejected."
  (let ((result (elinit--validate-entry '("cmd" :success-exit-status (256)))))
    (should (stringp result))
    (should (string-match-p ":success-exit-status code .* is outside valid range"
                            result))))

(ert-deftest elinit-test-validate-success-exit-status-valid-boundaries ()
...
(ert-deftest elinit-test-validate-unit-file-delegates-boolean ()
...
(ert-deftest elinit-test-validate-unit-file-delegates-type-gate ()
...
(ert-deftest elinit-test-validate-unit-file-delegates-self-dependency ()
...
(ert-deftest elinit-test-validate-unit-file-delegates-empty-command ()
...
(provide 'elinit-test-validation)
;;; elinit-test-validation.el ends here
