...
Invalid entries are skipped at start/reconcile and shown as ~invalid~ in dashboard
...
- ~enable ID~: unit should start on next ~supervisor-start~ / ~reconcile~.
...
- Reconcile skips disabled units but does not stop disabled units that
  were manually started (preserves session-only starts).
...
- Stops masked entries (mask overrides manual start)
- Stops entries newly disabled (but preserves disabled entries that were
  manually started, per the systemctl model)
...
- does NOT affect other units (no global convergence; use ~reconcile~ for that)
...
- available in dashboard transient menu under "System" group (~R~)
...
| ~supervisor-watch-config~ | ~nil~ | Config file watch/reconcile |
...
| ~M-x supervisor-reconcile~ | Reconcile running state against config |
...
- Reconcile action model (~start~, ~stop~, ~noop~, ~skip~): [[#runtime-overrides-and-reconciliation][Runtime Overrides and Reconciliation]]
- Reload unit (~reload~, ~R~ in dashboard): [[#cli-handbook-sbinsupervisorctl][CLI (~sbin/supervisorctl~)]]
