...
in your authority roots (default ~~/.config/supervisor.el/~).
...
Unit files found in any active root are merged with ~supervisor-programs~ at
startup.  Unit-file entries take precedence on ID collision with legacy entries.
...
~:no-restart~, ~:logging~, ~:oneshot-blocking~, ~:oneshot-async~, ~:oneshot-timeout~,
~:tags~).
...
*** Step 2: Configure authority roots (optional)

The default authority path is usually sufficient:

#+begin_src emacs-lisp
;; Default: vendor → system → user (highest precedence)
(setq supervisor-unit-authority-path
      '("/usr/lib/supervisor.el/"
        "/etc/supervisor.el/"
        "~/.config/supervisor.el/"))
#+end_src

Only customize this if you need additional or different roots.

*** Step 3: Create unit files

For each service you want to extract, create a file in your highest-precedence
authority root (default ~~/.config/supervisor.el/~).  The filename should be
~ID.el~ (e.g., ~nm-applet.el~).  Each file contains a single plist:
...
- *No RemainAfterExit.*  Supervisor has no equivalent of
  ~RemainAfterExit=yes~ (keeping a unit in "active" state after the
  process exits).  Oneshot completion is tracked in
  ~supervisor--oneshot-completed~ with exit codes, but the service
  status shows ~done~ rather than ~active (exited)~.
...
- reports the authority root in human output and JSON
- non-interactive: launches ~$VISUAL~ or ~$EDITOR~
- JSON: ~{"path": "...", "root": "...", "created": true/false}~

~daemon-reload~:

- no arguments
- re-reads ~supervisor-programs~ and unit files from all authority roots,
  rebuilds the internal plan
...
- Async oneshots (~:oneshot-async~, ~:oneshot-blocking~): [[#service-definition-handbook-supervisor-programs][Service Definition]], [[#startup-and-lifecycle-model][Startup and Lifecycle Model]]
