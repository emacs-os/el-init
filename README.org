...
[[https://gist.github.com/cypherpunk2001/e686727a6d88c17c557003e73a9c020c][https://img.shields.io/endpoint.svg?url=https://gist.githubusercontent.com/cypherpunk2001/e686727a6d88c17c557003e73a9c020c/raw/elinit-tests.json]]

* elinit

[[file:el-init-branding.png]]

*A proper process supervisor with a systemd-inspired dependency model.*

~elinit~ is an Emacs Lisp-based, systemd-inspired init and
process-supervision solution.  Whether you are building a full-send Emacs-OS, or
simply want to orchestrate your desktop session services from within EXWM,
or keep project daemons running while you work,
elinit has you covered.  Use ~M-x elinit~ for the interactive
...
- [[#dashboard-handbook-m-x-elinit][Dashboard]] - the interactive UI
- [[#cli-handbook-sbinelinitctl][CLI]] - terminal control with ~elinitctl~
...
- [[#service-management-systems-comparison][Systems Comparison]] - elinit vs systemd vs runit vs s6
...
~M-x elinit~ opens an interactive dashboard for managing services:

#+CAPTION: Elinit dashboard
...
- Dashboard visual customization (status/type/separator faces): [[#dashboard-handbook-m-x-elinit][Dashboard (~M-x elinit~)]]
...
- Graceful vs hard stop (~elinit-stop~ vs ~elinit-stop-now~): [[#startup-and-lifecycle-model][Startup and Lifecycle Model]], [[#command-reference][Command Reference]]
- Hyphen-prefixed IDs in CLI (use ~--~ separator): [[#cli-handbook-sbinelinitctl][CLI (~sbin/elinitctl~)]]
- JSON contracts (status/verify/list-timers + error shape): [[#cli-handbook-sbinelinitctl][CLI (~sbin/elinitctl~)]]
...
- Structured log formats (~:log-format text|binary~, ~elinit-log-format-binary-enable~): [[#service-definition][Service Definition]], [[#persistence-and-files][Persistence and Files]], [[#dashboard-handbook-m-x-elinit][Dashboard (~M-x elinit~)]]
...
- Reload unit (~reload~, ~u~ in dashboard): [[#cli-handbook-sbinelinitctl][CLI (~sbin/elinitctl~)]]
- Reset failed state (~reset-failed~, ~F~ in dashboard): [[#cli-handbook-sbinelinitctl][CLI (~sbin/elinitctl~)]], [[#dashboard-handbook-m-x-elinit][Dashboard (~M-x elinit~)]]
...
- Wrapper help vs no-command usage text: [[#cli-handbook-sbinelinitctl][CLI (~sbin/elinitctl~)]]
...
- Systemd example conversions (unit file to elinit config): [[#service-management-systems-comparison][Service Management Systems Comparison]]
...
;; Skipped directives (no elinit equivalent):
  LimitNOFILE=65536 -- no elinit equivalent
#+end_example

The skipped-directive summary at the end lists every line that had no
elinit equivalent, so you can review what was dropped.
