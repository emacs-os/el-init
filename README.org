...
[[https://gitlab.com/telecommuter/el-init][https://img.shields.io/badge/mirror-GitLab-orange.svg]]
[[https://codeberg.org/telecommuter/el-init][https://img.shields.io/badge/mirror-Codeberg-2185D0.svg]]
[[https://github.com/melpa/melpa/pull/9847][https://img.shields.io/badge/MELPA-pending-purple.svg]]
...
- *Optional PID1 patchset* -- static build instruction files (Arch
  ...
  PID1 lifecycle hooks (~static-builds/~).
...
[[file:assets/you-really-should-read-this-small.png]]

See [[file:RETROSPECTIVE.md][RETROSPECTIVE.md]] for the full disclosure
and the human author's unfiltered opinions on AI-driven development.
Some of those opinions may surprise you.
...
- [[#pid1---for-the-init-hackers-tinkerers-and-adventurers-among-us][PID1 - for the init hackers, tinkerers and adventurers among us]] - all docs for this crazy idea are in =./static-builds=
...
These commands manage elinit target state only.  They do not directly
issue kernel poweroff or reboot syscalls.
...
1. You usually do *not* need one if your initramfs already handles core early
   boot responsibilities before handing off to Emacs PID1 (mount, dev, fsck,
   networking bootstrap, runtime dirs).
2. You *do* need one when those responsibilities are not handled before
   handoff and must be done deterministically before normal services.
3. In that case, define an explicit early oneshot unit and wire it near the
   start of the target graph (for example required by ~basic.target~ and
   blocking).
...
This keeps boot sequencing explicit, testable, and visible through the same
supervisor model as every other unit.

When you need shutdown or reboot scripts:

1. You usually do *not* need one if stopping managed units is enough.  In PID1
   mode, signal-driven shutdown/reboot hooks already call ~elinit-stop-now~.
2. You *do* need one if you require deterministic pre-transition tasks under
   supervisor control (for example cleanup or state export).
3. In that case, attach blocking oneshot units to ~shutdown.target~,
   ~poweroff.target~, and ~reboot.target~, then trigger transition
   targets explicitly with ~elinitctl init --yes 0~ (poweroff path) or
   ~elinitctl init --yes 6~ (reboot path).
...
:PROPERTIES:
:CUSTOM_ID: pid1-for-the-init-hackers-tinkerers-and-adventurers-among-us
:END:

All documentation related to this crazy idea is in [[file:./static-builds][./static-builds]].
...
- PID1 mode, static builds, Emacs patches, deterministic early boot units: [[#customization-reference][Customization Reference]]
