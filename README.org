...
| ~:kill-signal~ | symbol | ~SIGTERM~ | Graceful stop signal for this unit |
| ~:kill-mode~ | symbol | ~process~ | ~process~ or ~mixed~, see Stop Semantics |
| ~:remain-after-exit~ | boolean | ~nil~ | ~oneshot~ only: latch active on success |
| ~:success-exit-status~ | int, signal, or list | ~nil~ | ~simple~ only: extra clean exit criteria |
...
- ~:environment~ must be an alist of ~(KEY . VALUE)~ string pairs.
- ~:environment-file~, ~:exec-stop~, and ~:exec-reload~ must be string or proper
  list of strings.
- ~:restart-sec~ must be a non-negative number.
- ~:description~ must be a string.
- ~:documentation~ must be a string or proper list of strings.
- ~:before~ and ~:wants~ must be a string or proper list of strings.
...
- ~:success-exit-status~ items must be integers or recognized signal names;
  rejected for ~oneshot~ type.
...
(~supervisorctl reload ID~, dashboard ~u~) runs the reload commands
...
Per-unit stop (~supervisorctl stop ID~, dashboard ~x~):
...
| ~F~ | Reset failed state for selected service |
...
The following systemd directives are not implemented and have no planned
implementation.  They are listed here explicitly for auditability.

| Category | Directives |
|----------+------------|
| Default dependencies | ~DefaultDependencies~ |
| Boot/target/install graph | ~WantedBy~, ~RequiredBy~, ~Also~, ~Alias~, target units |
| Cgroup hardening | ~ProtectSystem~, ~ProtectHome~, ~NoNewPrivileges~, ~PrivateTmp~, ~PrivateNetwork~, ~PrivateDevices~, namespace knobs |
| Resource limits | ~MemoryMax~, ~CPUQuota~, ~TasksMax~, ~LimitNOFILE~, ~LimitCORE~ |
| OOM and failure policy | ~OOMPolicy~, ~FailureAction~, ~SuccessAction~, ~StartLimitBurst~, ~StartLimitIntervalSec~ |
| Watchdog | ~WatchdogSec~, ~WatchdogSignal~ |
| Credentials and security | ~LoadCredential~, ~SetCredential~, ~DynamicUser~, ~User~, ~Group~ |
| Notification | ~Type=notify~, ~NotifyAccess~ |
| Socket activation | ~Type=socket~, socket units |
| D-Bus activation | ~Type=dbus~, ~BusName~ |
| Paths and mounts | Path units, mount units, automount units |
| Timers | systemd timer units (supervisor has its own timer subsystem) |
| Slices and scopes | Slice units, scope units |
| Conditions | ~ConditionPathExists~, ~AssertPathExists~, etc. |
