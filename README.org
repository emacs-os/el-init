...
- Stage and tag filters apply only to the service section; the timer section
...
~EXIT~, ~MISS~.  This matches the columns of ~supervisorctl list-timers~.
...
A service health summary row appears between the service section and the
timer section, labeled "services" to indicate it counts only service
rows (not timers).
...
Built-in ~logrotate~ and ~log-prune~ units appear in stage4 as oneshot
services.  When the timer subsystem is enabled (the default), timers
~logrotate-daily~ and ~log-prune-daily~ schedule those units automatically.
