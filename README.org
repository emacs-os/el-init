...
- *DAG-based startup ordering* -- topological sort with ~:after~, ~:requires~,
  ~:wants~, ~:before~ dependency edges.  Targets (~default.target~,
...
- *3-tier authority model* -- ~/usr/lib/elinit/~ (vendor),
  ~/etc/elinit/~ (admin), =~/.config/elinit/= (user).
...
- [[#running-as-pid-1][Running as PID 1]] - optional static builds and Emacs patches for full Emacs-OS
...
=~/.config/elinit/=).  For example:

#+begin_src emacs-lisp
;; ~/.config/elinit/xhost.el
(:id "xhost"
...
managing service lifecycle, policies, and inspecting state.
...
(setq elinit-unit-authority-path
      '("/usr/lib/elinit/"    ; Tier 1: vendor (lowest precedence)
        "/etc/elinit/"        ; Tier 2: system admin
        "~/.config/elinit/")) ; Tier 3: user (highest precedence)
#+end_src
...
#+begin_src emacs-lisp
;; /usr/lib/elinit/polkit.el  (Tier 1, vendor)
(:id "polkit"
...
#+begin_src emacs-lisp
;; ~/.config/elinit/polkit.el  (Tier 3, user)
(:id "polkit"
 :command "/usr/lib/polkit-gnome/polkit-gnome-authentication-agent-1"
 :type simple
 :wanted-by ("graphical.target")
...
(setq elinit-unit-authority-path
      '("/nonexistent/root/"          ; Silently skipped (does not exist)
        "/etc/elinit/"         ; Active if exists
        "~/.config/elinit/"))  ; Active if exists
#+end_src
...
#+begin_src emacs-lisp
;; /usr/lib/elinit/backup.el  (Tier 1, vendor -- valid)
(:id "backup"
...
#+begin_src emacs-lisp
;; ~/.config/elinit/backup.el  (Tier 3, user -- INVALID: missing :command)
(:id "backup"
 :type simple)
#+end_src
...
=~/.config/elinit/=).  The filename should be ~ID.el~ (e.g.,
~nm-applet.el~):
...
By default, elinit does not show minibuffer messages for process failures
...
| Variable | Default | Purpose |
| ~elinit-unit-authority-path~ | ~'("/usr/lib/elinit/" "/etc/elinit/" "~/.config/elinit/")~ | Authority roots (low to high precedence) |
...
#+begin_example
sbin/elinit-import /etc/systemd/system/foo.service > ~/.config/elinit/units/foo.el
#+end_example
...
* Running as PID 1
:PROPERTIES:
:CUSTOM_ID: running-as-pid-1
:END:

For a full-send ~emacs --pid1~ Emacs-OS setup, the ~static-builds/~
directory contains everything needed to build a statically linked Emacs
binary that can run as PID 1.

The directory provides:

- *Build instruction files* -- Arch Linux PKGBUILDs and Nix derivations for
  eight build variants (three vanilla static, one PID1-patched, each in both
  Arch and Nix).
- *Emacs patches* (~static-builds/patches/~) -- three patches against Emacs
  30.2 that add a ~--pid1~ command-line flag, zombie reaping via non-blocking
  ~waitpid(2)~ on ~SIGCHLD~, signal-driven shutdown hooks
  (~pid1-poweroff-hook~, ~pid1-reboot-hook~), and support for Emacs Lisp
  init scripts (~rc.boot.el~, ~rc.shutdown.el~) that handle early-boot
  mounts, device setup, and clean shutdown -- all in Lisp.
- *Example init scripts* (~static-builds/scripts/~) -- ready-to-adapt
  ~rc.boot.el~ and ~rc.shutdown.el~ templates covering /proc, /sys, /dev
  mounts, filesystem checks, udev, networking, and getty spawning.

The PID1-patched variants bundle elinit and its prebuilt C helpers directly
into the package, so the resulting binary needs no compiler, no user
init files, and no =~/.emacs= -- the packaged ~site-start.el~ bootstrap
wires elinit into ~pid1-boot-hook~ automatically when ~--pid1~ is passed.

See ~static-builds/README.md~ for the full administrator guide, including
build commands, file placement, first-boot validation, and disable gates.

** Getting Started

Start with a minimal source-based distribution such as [[https://kisslinux.org][KISS Linux]], Void
Linux (musl), or Alpine Linux -- anything where you control the kernel,
bootloader, and ~/etc~.  The steps below assume you are preparing a KISS
Linux target root from a working Arch system via chroot.  Adapt paths
if your host or target differ.

*** Prerequisites

On your Arch host, make sure you have:

- A partitioned and formatted target disk mounted at ~/mnt~
- A KISS Linux (or similar) base installed in ~/mnt~
- A static kernel built and installed at ~/mnt/boot/vmlinuz~
- GRUB installed to the target disk

*** 1. Build the PID1-patched Emacs (on the host)

Build the static binary on your Arch host and copy it into the target
root.  The binary is statically linked so it has no runtime dependencies
on the target.
...
#+begin_src bash
install -Dm755 emacs-nox-static-elinit-patched-for-pid1-*/usr/bin/emacs \
  /mnt/usr/bin/emacs
#+end_src
...
#+begin_src bash
file /mnt/usr/bin/emacs   # should say "statically linked"
#+end_src

*** 2. Install init scripts (in the chroot)

Enter the target chroot and place the boot and shutdown scripts:
...
Copy the example templates from the el-init source tree (accessible from
the host or pre-copied into the chroot):

#+begin_src bash
cp static-builds/scripts/rc.boot.el.example     /lib/init/rc.boot.el
cp static-builds/scripts/rc.shutdown.el.example  /lib/init/rc.shutdown.el
chmod 644 /lib/init/rc.boot.el /lib/init/rc.shutdown.el
#+end_src

Edit ~/lib/init/rc.boot.el~ to match your hardware and preferences -- the
example template handles /proc, /sys, /dev, /run, devpts, shm, filesystem
checks, hostname, sysctl, udev, and getty spawning.  Remove or adjust
sections you do not need.

*** 3. Seed your service units

Create the system unit directory and drop in unit files for the services
you want elinit to manage at boot:

#+begin_src bash
install -d /etc/elinit.el

cat > /etc/elinit.el/sshd.el <<'EOF'
(:id "sshd"
...
cat > /etc/elinit.el/dhcpcd.el <<'EOF'
(:id "dhcpcd"
 :command "/usr/sbin/dhcpcd -BM"
 :type simple
 :enabled t
 :wanted-by ("multi-user.target")
 :restart on-failure
...
*** 4. Point GRUB at Emacs

Still inside the chroot, configure the bootloader to hand PID 1 to
Emacs.  Add ~init=/usr/bin/emacs~ to the kernel command line and pass
~--pid1~ after the ~--~ separator (everything after ~--~ goes to the
init process, not the kernel):

#+begin_src bash
# /etc/default/grub
GRUB_CMDLINE_LINUX="init=/usr/bin/emacs"
GRUB_CMDLINE_LINUX_DEFAULT="quiet -- --pid1"
#+end_src
...
Select the target disk in GRUB.  The kernel hands PID 1 to Emacs.  The
~--pid1~ flag enables zombie reaping, signal-driven shutdown hooks, and
the ~pid1-boot-hook~ that loads ~rc.boot.el~ and starts elinit.  Verify
after login:

#+begin_src bash
ps -p 1 -o comm,args       # should show emacs --pid1
elinitctl status            # should show running services
elinitctl list-units        # should list your unit files
#+end_src

SIGTERM or ~reboot(8)~ triggers ~pid1-poweroff-hook~ /
~pid1-reboot-hook~, which loads ~rc.shutdown.el~, stops all services,
unmounts filesystems, and halts or reboots cleanly.
...
- PID1 mode, static builds, Emacs patches, init scripts: [[#running-as-pid-1][Running as PID 1]], [[#customization-reference][Customization Reference]]
