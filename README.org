...
~svc.20250101-120000~) are automatically protected by two mechanisms:

1. *Children guard* --- once the log has been rotated at least once, the
   rotated child confirms the parent as an active file.
2. *Open-file guard* --- when ~fuser~ is available, files currently held
   open by a process (e.g. the logd writer) are never deleted, even if
   they have no rotated children yet.

If neither guard applies (service is stopped and has never been rotated),
use ~--protect-id~ for explicit protection.
