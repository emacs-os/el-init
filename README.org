...
- [[#supervisor-import][systemd Import]] - automated conversion of systemd service files
...
Write the result directly to a unit file:

#+begin_example
sbin/supervisor-import /etc/systemd/system/foo.service > ~/.config/supervisor/units/foo.el
#+end_example

Or use ~--output-dir~ to write automatically (the output filename is
derived from the service name):

#+begin_example
sbin/supervisor-import --output-dir ~/.config/supervisor/units /etc/systemd/system/foo.service
#+end_example
...
The output always includes ~:enabled t~.
