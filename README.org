...
*Manage your session processes from Emacs.*

If you run Emacs as your window manager (EXWM), keep a long-lived daemon, or
just want your tray applets and background services to start automatically and
restart when they crash---~supervisor.el~ is for you.
...
~supervisor.el~ is a *per-user session supervisor*. It shines when:

- Emacs is your window manager (EXWM) and you need to start X utilities, tray
  applets, and background services in the right order.

- You run Emacs as a daemon and want session services that live as long as
  Emacs does.

- You want your services defined in one place, in Elisp, versioned with your
  dotfiles.

It is not a system init replacement---it manages your session, not your system.
