...
[[https://gist.github.com/cypherpunk2001/e686727a6d88c17c557003e73a9c020c][https://img.shields.io/endpoint.svg?url=https://gist.githubusercontent.com/cypherpunk2001/e686727a6d88c17c557003e73a9c020c/raw/supervisor-el-tests.json]]

* supervisor.el

[[file:supervisor_logo_small.png]]

*A proper process supervisor with a systemd-inspired dependency model.*

~supervisor.el~ is an Emacs Lisp-based, systemd-inspired init and
supervisor solution.  Whether you are building a full-send Emacs-OS, or
simply want to orchestrate your desktop session services from within EXWM,
or keep project daemons running while you work,
supervisor.el has you covered.  Use ~M-x supervisor~ for the interactive
dashboard, or ~supervisorctl~ from the terminal.
...
- *Structured event system* -- hook-based events for process-exit,
  ...
  Full programmatic integration via ~supervisor-event-hook~.
...
- *3-tier authority model* -- ~/usr/lib/supervisor.el/~ (vendor),
  ~/etc/supervisor.el/~ (admin), =~/.config/supervisor.el/= (user).
...
- *Dashboard* -- ~M-x supervisor~ for a live view with lifecycle actions,
...
- *systemd import* -- ~sbin/supervisor-import~ converts systemd ~.service~
  files to supervisor unit plists.
...
- [[#topical-index][Topical Index]] - targeted lookup for specific behaviors
...
- ~:log-format~ must be ~text~ or ~binary~.  ~binary~ is rejected unless
  ~supervisor-log-format-binary-enable~ is non-nil.  ~:log-format~ is rejected
  for ~:type target~.
...
entries.  This reduces noise for operators who use supervisor as a
...
| ~supervisor-section-separator~ | Section header/separator rows |
...
variable ~supervisor-log-format-binary-enable~ to be non-nil; units
...
**** ~supervisorctl journal~ Command

The ~journal~ command reads and displays structured log records:

#+begin_example
supervisorctl journal -u my-service
supervisorctl journal -u my-service -n 20
supervisorctl journal -u my-service -p err
supervisorctl journal -u my-service --since 1708098896 --until 1708099000
supervisorctl journal -fu my-service
supervisorctl journal --json -u my-service
#+end_example
...
The dashboard (~supervisor-dashboard-view-log~) also auto-detects binary
...
#+begin_example
sbin/supervisor-log-prune --log-dir /path/to/logs --vacuum --vacuum-max-total-bytes 1073741824
#+end_example
...
| ~supervisor-log-follow-interval~ | ~1.0~ | Seconds between journal follow poll cycles |
| ~supervisor-log-default-max-bytes~ | ~(* 5 1024 1024)~ (5 MiB) | Default tail byte cap when log decoding has no explicit record count |
...
| ~supervisor-log-format-binary-enable~ | ~nil~ | Gate variable: allow ~:log-format binary~ in unit files |
...
| ~supervisor-dashboard-log-view-record-limit~ | ~1000~ | Maximum decoded records shown by ~supervisor-dashboard-view-log~ |
| ~supervisor-auto-refresh-interval~ | ~2~ | Auto-refresh cadence |

** CLI Options

| Variable | Default | Purpose |
| ~supervisor-cli-follow-max-age~ | ~3600~ | Seconds before orphaned journal follow sessions are cleaned up |
...
| ~M-x supervisor-daemon-reload~ | Reload unit definitions from disk into memory without runtime changes |
...
| ~M-x supervisor-dashboard-toggle-targets~ | Toggle visibility of target entries |
| ~M-x supervisor-dashboard-toggle-init-targets~ | Toggle visibility of init-transition targets (rescue/shutdown/etc) |
| ~M-x supervisor-dashboard-toggle-auto-refresh~ | Toggle auto-refresh |
| ~M-x supervisor-dashboard-toggle-proced-auto-update~ | Toggle Proced auto-update mode |
...
| ~M-x supervisor-dashboard-daemon-reload~ | Reload all unit definitions from disk and refresh dashboard |
...
| Command | Purpose |
| ~M-x supervisor-edit-mode~ | Unit edit minor mode (normally enabled automatically by ~supervisor-dashboard-edit~) |
...
- Structured log formats (~:log-format text|binary~, ~supervisor-log-format-binary-enable~): [[#service-definition][Service Definition]], [[#persistence-and-files][Persistence and Files]], [[#dashboard-handbook-m-x-supervisor][Dashboard (~M-x supervisor~)]]
...
- Bubblewrap sandboxing (~:sandbox-profile~, ~:sandbox-network~, bind mounts, raw mode): [[#sandbox][Process Sandbox (bubblewrap)]], [[#service-definition][Service Definition]]
- Run service as another user/group (~:user~, ~:group~, trust gate): [[#security][Security]], [[#service-definition][Service Definition]]
...
are applied via a small C helper (~supervisor-rlimits~) that calls
...
- CLI ~supervisorctl describe ID~ includes a "Resource limits" section listing
  each non-nil limit.
...
#+begin_src emacs-lisp
;; ~/.config/supervisor/units/mydb.el
(:id "mydb"
...
- The ~supervisor-rlimits~ C helper is compiled as part of
  ~make -C libexec check~.  If it fails to compile (e.g., missing RLIMIT
  constants on an exotic platform), units with limit keys will fail validation.
...
| *Logging* | | | | |
| Integrated logging daemon | yes (journald, structured binary) | yes (svlogd, plain text) | yes (s6-log, plain text) | yes (supervisor-logd, text or binary) |
...
:PROPERTIES:
:CUSTOM_ID: example-conversions-systemd-to-supervisor
:END:
...
** Automated Import (~sbin/supervisor-import~)
:PROPERTIES:
:CUSTOM_ID: supervisor-import
:END:

The ~sbin/supervisor-import~ script automates the translation shown above.
It reads a systemd ~.service~ file and prints the equivalent supervisor
plist to stdout.

#+begin_example
sbin/supervisor-import /etc/systemd/system/foo.service
#+end_example

Redirect to save as a unit file:

#+begin_example
sbin/supervisor-import /etc/systemd/system/foo.service > ~/.config/supervisor/units/foo.el
#+end_example
...
All directives from the [[#example-conversions-systemd-to-supervisor][translation table]] are supported: ~Description~,
...
~User~, ~Group~, ~WantedBy~, and ~RequiredBy~.
...
Unsupported systemd directives (such as ~LimitNOFILE~ or ~CapabilityBoundingSet~)
...
#+begin_example
$ sbin/supervisor-import /etc/systemd/system/webapp.service
#+end_example
...
#+begin_example
Warning: unknown directive: LimitNOFILE=65536

;; Skipped directives (no supervisor equivalent):
  LimitNOFILE=65536 -- no supervisor equivalent
#+end_example

The skipped-directive summary at the end lists every line that had no
supervisor equivalent, so you can review what was dropped.
