...
~supervisor.el~ is a mostly Emacs Lisp process supervisor. The core runtime and
UI are in Elisp, with small ~libexec~ helpers for logging and user/group
execution robustness. ~M-x supervisor~ is the main interface for day-to-day
management: start/stop, policy changes, dependency visibility, and log
inspection.
...
- *Dashboard*: ~M-x supervisor~ provides a live view for lifecycle actions,
  policy changes, dependency inspection, and log viewing.

- *Staged startup and dependencies*: Four startup stages plus ordering
  (~:after~) and pull-in requirements (~:requires~).

- *Service types and restart policies*: Long-running ~simple~ services and
  run-once ~oneshot~ tasks, with explicit restart policy selection.

- *Persistent runtime overrides*: Enable/disable, mask/unmask, restart-policy,
  and logging overrides persist across restarts.

- *Authority-tier unit files*: Shipped defaults can be overridden by
  higher-precedence user tiers, with daemon-reload support.

- *CLI*: ~sbin/supervisorctl~ supports scripting and remote use (human and JSON
  output).

- *Timer subsystem (experimental)*: Calendar and relative timers for oneshot
  services, including persistent catch-up semantics.

- *Logging maintenance*: External log writers plus rotate/prune tooling for
  bounded log growth.
