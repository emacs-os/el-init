...
The wrapper validates transport options:

- `--socket` and `--server-file` are mutually exclusive (error if both specified)
- `--server-file` (TCP transport) emits a warning to use localhost-only binding
- `--strict` flag converts warnings to fatal errors
...
If socket or auth files may have been exposed:

1. Stop supervisor: `supervisorctl stop`
2. Kill Emacs server: `emacsclient -e '(kill-emacs)'`
3. Remove and regenerate server auth files
4. Restart Emacs server with correct permissions
5. Verify socket permissions before reconnecting
