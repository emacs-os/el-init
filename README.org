...
~supervisor.el~ is a service supervisor in Emacs Lisp, with small C helpers for
privilege dropping and log management.  It follows a familiar systemd-like
interface --- service types, dependencies, restart policies --- adapted for
Emacs.  ~M-x supervisor~ is the main interface: start, stop, inspect, and
manage everything from inside Emacs.

Define your service units in Elisp, organize them into startup stages (like
runit), declare dependencies, and let supervisor handle the rest.
