...
*Scope:* This is a user-session supervisor, not an init system or PID1 replacement.

* Why supervisor.el

When using Emacs as a window manager, you need something to start your session
services (nm-applet, polkit agent, compositor, etc.). You could use systemd
user units, but then you're managing your Emacs session with two different
tools, debugging across two log systems, and dealing with startup ordering
between processes that systemd knows about and Emacs.

supervisor.el keeps everything in one place:
- *Transparent:* Plain text logs, explicit config, no magic
- *Inspectable:* See resolved config, dependency graph, and timing with a keystroke
- *Interactive:* Override restart policy, toggle logging, start/kill from dashboard
- *Integrated:* Your session config lives with your Emacs config

It deliberately trades industrial-strength process management for simplicity
and debuggability within a single user session.
...
(add-hook 'kill-emacs-hook (lambda () (supervisor-stop (lambda ()))))
#+end_src
...
| =M-x supervisor-reload=  | Reconcile: stop removed/disabled, start new   |
...
| =M-x supervisor-dry-run= | Validate and show startup order (no start)    |
...
- *Disabled entry*: immediately ready (skipped)
- *Start failure*: immediately ready (allows dependents to proceed)
...
=supervisor-stop= sends SIGTERM to all processes and returns immediately.
A timer handles the graceful shutdown window: after
=supervisor-shutdown-timeout= seconds (default: 3), any survivors are sent
SIGKILL.  If you need to block until completion (for example in
=kill-emacs-hook=), pass a callback to =supervisor-stop= and wait on it in
your own code.
...
| Key | Action                                             |
|-----+----------------------------------------------------|
| e   | Toggle enabled (takes effect on next start)        |
| f   | Cycle stage filter (all -> early -> ... -> all)    |
| t   | Cycle tag filter (all -> tag1 -> tag2 -> all)      |
...
| L   | View log file                                      |
| ?   | Show resolved config for entry                     |
| d   | Show dependencies for entry at point               |
| D   | Show full dependency graph for all entries         |
| B   | Show startup blame view (sorted by duration)       |
| p   | Open =M-x proced=                                    |
...
| =:tags=            | list        | nil       | Tags for dashboard filtering             |
...
| =supervisor-stage-timeout=       | nil                        | Stage timeout in seconds (nil = none) |
| =supervisor-max-concurrent-starts= | nil                        | Max parallel starts (nil = unlimited) |
| =supervisor-verbose=             | nil                        | Log all events (stages, starts, unlocks) |
| =supervisor-log-to-file=         | nil                        | Also write events to supervisor.log  |
| =supervisor-watch-config=        | nil                        | Watch init file and auto-reload      |

* Hooks

| Hook                          | Arguments           | When                                |
|-------------------------------+---------------------+-------------------------------------|
| =supervisor-cleanup-hook=       | none                | Before killing processes on shutdown |
| =supervisor-stage-start-hook=   | stage-name          | When a stage begins                 |
| =supervisor-stage-complete-hook= | stage-name          | When a stage completes              |
| =supervisor-process-exit-hook=  | id, status, code    | When any process exits              |

The =supervisor-process-exit-hook= receives:
- =id= - process identifier (string)
- =status= - symbol: =exited= (normal exit), =signal= (killed by signal), or =unknown=
...
(add-hook 'after-init-hook #'supervisor-start)
;; If you need to block, use the callback to perform sync teardown.
(add-hook 'kill-emacs-hook
          (lambda () (supervisor-stop (lambda ()))) )
#+end_src
