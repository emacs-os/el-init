...
This project provides deterministic process supervision, staged startup,
dependency management, crash recovery, and observability from within Emacs.

It can serve as an alternative to ~systemctl --user~ on systemd-based systems,
and is particularly well-suited to EXWM or highly customized desktop
environments where multiple independent services must be coordinated reliably.

It is also useful for users who already run Emacs as a long-lived daemon and
prefer to configure and manage their remaining session services, background
tasks, or scheduled jobs directly in Emacs Lisp rather than through external
service managers.

* Navigation

- [[#about-supervisorel][About supervisor.el]]
- [[#codebase-layout][Codebase Layout]]
- [[#requirements][Requirements]]
- [[#install-and-enable][Install And Enable]]
- [[#public-commands][Public Commands]]
- [[#cli-control-plane-supervisorctl][CLI Control Plane (~supervisorctl~)]]
- [[#configuration-model][Configuration Model]]
- [[#startup-execution-model][Startup Execution Model]]
- [[#entry-lifecycle-state-machine][Entry Lifecycle State Machine]]
- [[#process-lifecycle-and-restart][Process Lifecycle And Restart]]
- [[#reload-reconciler][Reload Reconciler]]
- [[#stop-semantics][Stop Semantics]]
- [[#dashboard-m-x-supervisor][Dashboard (~M-x supervisor~)]]
- [[#logging][Logging]]
- [[#config-file-watch][Config File Watch]]
- [[#structured-events-and-hooks][Structured Events And Hooks]]
...
| File | Role |
| ~supervisor.el~ | Facade. Requires all modules and provides ~supervisor~ |
| ~supervisor-core.el~ | Parsing, validation, planning, scheduler, runtime state, process lifecycle, persistence |
| ~supervisor-dashboard.el~ | Dashboard UI, keymaps, rendering, dashboard actions |
| ~supervisor-cli.el~ | CLI parsing/dispatch/formatting and wrapper transport adapter |
...
| ~M-x supervisor-migrate-config~ | Show canonical schema v1 view of current config |
| ~M-x supervisor-overrides-load~ | Reload persisted runtime overrides from disk |
| ~M-x supervisor-overrides-save~ | Save current runtime overrides to disk |
| ~M-x supervisor-overrides-clear~ | Clear runtime overrides in memory and on disk |
...
| ~supervisor-dashboard-show-header-hints~ | ~nil~ | Show key hints in dashboard header |
...
This is a user-session supervisor, not a hardened system init.  The CLI
control plane (~supervisorctl~) uses ~emacsclient --eval~ to execute Elisp
on the Emacs server.  Any principal that can connect to the server socket
can execute arbitrary code as the Emacs user.

Trust boundary: the Emacs server socket is the security perimeter.

*** Threat Model

| Threat | Mitigation |
| Local unprivileged user on same host | Unix socket permissions (Emacs creates socket dir mode 0700) |
| Compromised process as same user | Out of scope - same-user access is trusted |
| Misconfigured TCP server exposure | Use local sockets; if TCP required, bind localhost only |

*** Assumptions

This package assumes default Emacs server behavior:

- Socket directory created with mode 0700 (owner-only access)
- No TCP server enabled unless explicitly configured

Custom ~server-auth-dir~, ~server-socket-dir~, or TCP server configurations
are the operator's responsibility to secure.  The wrapper does not validate
filesystem permissions on these paths.
...
#+begin_src bash
# Default behavior - uses local Unix socket
supervisorctl status
#+end_src

If TCP is required:

- Bind to localhost only: ~(setq server-host "127.0.0.1")~
- Keep ~server-auth-dir~ private (mode 0700)
- Use explicit ~--server-file~ rather than ambient environment

For system-level deployments:

- Run Emacs under a dedicated service account
- Restrict socket directory permissions
- Consider MAC policy (SELinux/AppArmor) for socket access
