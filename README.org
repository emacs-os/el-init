...
| Set policy | ~supervisorctl restart-policy ID always~ | (edit unit file) | (no equivalent) |
| Set logging | ~supervisorctl logging ID on~ | (edit unit file) | Logging is automatic |
...
- *Explicit verb*: performs exactly one named operation on a named target.
...
No toggle/cycle mutators remain.  Legacy toggle commands
(~toggle-restart~, ~toggle-enabled~, ~toggle-mask~, ~toggle-logging~)
were removed in favour of explicit verb replacements.
...
| ~supervisor--save-overrides~ | Explicit verb | Writes override hash tables to ~supervisor-overrides-file~ |
| ~supervisor--load-overrides~ | Explicit verb | Reads override hash tables from ~supervisor-overrides-file~ |
| ~supervisor--clear-all-overrides~ | Explicit verb | Clears all override hash tables and deletes file |
...
| ~enable~ | ~supervisor--cli-cmd-enable~ | Explicit verb | puthash ~enabled-override~ + ~save-overrides~ | Yes |
| ~disable~ | ~supervisor--cli-cmd-disable~ | Explicit verb | puthash ~enabled-override~ + ~save-overrides~ | Yes |
| ~mask~ | ~supervisor--cli-cmd-mask~ | Explicit verb | puthash ~mask-override~ + ~save-overrides~ | Yes |
| ~unmask~ | ~supervisor--cli-cmd-unmask~ | Explicit verb | remhash ~mask-override~ + ~save-overrides~ | Yes |
| ~restart-policy~ | ~supervisor--cli-cmd-restart-policy~ | Explicit verb | puthash ~restart-override~ + ~save-overrides~ | Yes |
| ~logging~ | ~supervisor--cli-cmd-logging~ | Explicit verb | puthash ~logging~ + ~save-overrides~ | Yes |
...
| ~supervisor-dashboard-enable~ | Explicit verb | puthash/remhash ~enabled-override~ + ~save-overrides~ | Yes |
| ~supervisor-dashboard-disable~ | Explicit verb | puthash/remhash ~enabled-override~ + ~save-overrides~ | Yes |
| ~supervisor-dashboard-mask~ | Explicit verb | puthash ~mask-override~ + ~save-overrides~ | Yes |
| ~supervisor-dashboard-unmask~ | Explicit verb | remhash ~mask-override~ + ~save-overrides~ | Yes |
| ~supervisor-dashboard-set-restart-policy~ | Explicit verb | puthash/remhash ~restart-override~ + ~save-overrides~ | Yes |
| ~supervisor-dashboard-set-logging~ | Explicit verb | puthash/remhash ~logging~ + ~save-overrides~ | Yes |
...
| Enable | ~enable ID~ | ~p e~ | puthash ~enabled-override~ |
| Disable | ~disable ID~ | ~p d~ | puthash ~enabled-override~ |
| Mask | ~mask ID~ | ~p m~ | puthash ~mask-override~ |
| Unmask | ~unmask ID~ | ~p u~ | remhash ~mask-override~ |
| Restart policy | ~restart-policy ID POLICY~ | ~p r~ | puthash ~restart-override~ |
| Logging | ~logging ID on\vert{}off~ | ~p l~ | puthash ~logging~ |
...
call ~supervisor--save-overrides~ in both CLI and dashboard paths.
