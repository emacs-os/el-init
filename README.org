...
Run ~M-x supervisor-handbook~ to open this file in Emacs at any time.
...
| File | Purpose |
| ~supervisor.el~ | Entry point; loads all modules and provides feature ~supervisor~ |
...
Load order in entry point: core -> timer -> dashboard -> cli.

Module dependency rules:

- ~supervisor-core.el~ has no hard dependency on timer/dashboard/CLI modules.
  Optional timer and dashboard calls are guarded with ~fboundp~/~boundp~ so core
  can run standalone.
