...
- ~:sandbox-raw-args~ must be a list of strings.  Rejected unless
  ~supervisor-sandbox-allow-raw-bwrap~ is non-nil.
...
~--yes~ for confirmation.  Runlevels 1-5 do not require ~--yes~.
...
Timers targeting these IDs are rejected during validation with the
reason "init-transition target is not timer-eligible" and excluded
from the scheduler.
