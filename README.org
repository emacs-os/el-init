...
supervisor.el has you covered.  ~M-x supervisor~
opens the interactive dashboard from within your EXWM session;
~supervisorctl~ provides the same control from the terminal or scripts.
...
Below is what ~M-x supervisor~ shows after a default graphical boot with the
...
- *Your services* -- running, owned by their respective targets.  These are
  the units you defined.
- *logrotate and log-prune* -- built-in maintenance oneshots.  They show
  *unreachable* because no target in the startup path pulls them in.  They are
  not broken -- they activate only when their timers fire.  The ~pending~
  reason means they have not run yet.
...
- *Timers* -- trigger their target oneshot on a schedule.  ~LAST-RUN~ is ~-~
  until the first fire; ~NEXT-RUN~ shows the countdown to the next trigger.
...
init-transition targets (rescue, shutdown, poweroff, reboot, and
runlevel aliases) remain hidden by default.  Press ~V~ to include
init-transition targets.
...
| ~V~ | Toggle init-transition targets (rescue, shutdown, runlevel) |
| ~g~ | Refresh dashboard |
