...
- *Staged startup and dependencies*: four startup stages with ordering
  (~:after~, ~:before~) and pull-in (~:requires~, ~:wants~).
...
- *Timers*: calendar and relative timers for oneshot services with persistent
  catch-up semantics.
...
- [[#timer-subsystem-handbook][Timer Subsystem]] - scheduled oneshot execution
...
including the built-in daily log maintenance.
...
2. *Daily automatic path* --- a built-in ~logrotate~ oneshot unit and
   ~logrotate-daily~ timer run log rotation and pruning every day at
   03:00.  This runs automatically when the timer subsystem is enabled
   (the default).  After rotation completes, all log writers receive
   SIGHUP to reopen their files.
...
When the timer subsystem is enabled (the default), the built-in
~logrotate~ unit appears in stage4 as a oneshot service.
A user-created unit file with ~:id "logrotate"~ overrides the built-in
entry.  Similarly, a user timer with ~:id "logrotate-daily"~ in
~supervisor-timers~ overrides the built-in timer schedule.
...
- Stage model comparison (supervisor vs systemd targets vs runit stages): [[#service-management-systems-comparison][Service Management Systems Comparison]]
- Systemd command mapping (supervisorctl vs systemctl vs sv): [[#service-management-systems-comparison][Service Management Systems Comparison]]
...
- Timer miss reasons (~overlap~, ~disabled~, ~disabled-target~): [[#timer-subsystem-handbook][Timer Subsystem]], [[#dashboard-handbook-m-x-supervisor][Dashboard (~M-x supervisor~)]]
