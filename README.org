...
- *Logging* -- external log writer helpers with rotation and pruning for
  bounded growth (configurable per-file and total size caps).
...
- ~blame~
- ~logs [--tail N] [--] ID~
- ~journal [-u ID] [-n N] [-p PRIORITY] [--since TS] [--until TS] [--json]~
- ~ping~
- ~version~
...
| ~status~ | ~-~, ~exited~, ~signaled~ | Exit status (output events use ~-~) |
...
u16be unit_len
i32be exit_code
u8    exit_status      (0=none, 1=exited, 2=signaled)
u8[3] reserved         (0)
u32be payload_len
...
| ~-n N~ | Limit to last N records (default: 50) |
| ~-p err~ / ~-p info~ | Filter by priority |
| ~--since TS~ | Show records from timestamp (epoch integer or RFC3339) |
| ~--until TS~ | Show records up to timestamp |
