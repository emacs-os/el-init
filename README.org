...
[[file:assets/el-sloppo-small.png]]

This project is built with significant assistance from AI tooling.
...
Significant human effort drives the project direction, engineering
...
1. Generate a detailed, multi-phase requirements and specification plan
   file (~PLAN.md~), leaving no wiggle room or ambiguity.
2. Have Claude implement the plan.
3. Have Codex 5.3 Extra High review and audit the codebase against the
   plan, phase by phase.
4. Have Claude fix repeatedly until Codex is satisfied.
5. Maintain a detailed ~CLAUDE.md~ file (as demonstrated in this repo)
   that encodes project conventions, module structure, and coding
   standards so the model has persistent context.
6. Enforce a strong test policy -- syntax checking, compile-time
...
The author wishes to remain anonymous, as this project will likely
generate controversy.  It is released under GPL-3.0 -- fitting, given
that probably 99% of the Emacs Lisp code these models were trained on
was also GPL-3.0.
