...
Core orchestration is in Emacs Lisp.  Two bundled ~libexec~ helpers are used
for OS-level operations where direct process primitives are required:
~supervisor-logd~ (high-frequency log writing/rotation path) and
~supervisor-runas~ (privilege drop via ~setuid~/~setgid~).
...
work without write access to ~/ and /var/log.
...
~logrotate~ and ~log-prune~ units appear in stage4 as oneshot services.
