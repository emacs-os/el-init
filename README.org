...
Two paths keep log growth bounded end-to-end:
...
2. *Scheduled path* --- ~M-x supervisor-run-log-maintenance~ runs the
   full maintenance sequence asynchronously: rotate all active logs,
   signal writers to reopen, then prune.

The ~supervisor-run-log-maintenance~ command is suitable for periodic
invocation via a timer or manual use:

#+begin_example
M-x supervisor-run-log-maintenance
#+end_example

Both paths use the same lock file (~<log-dir>/.prune.lock~) for prune
