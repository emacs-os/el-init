...
Its core advantage is the interactive ~M-x supervisor~ dashboard: a fast,
Emacs-native control plane for lifecycle actions, policy changes, and live
status/log inspection in one place. If you only need CLI service control,
~systemctl --user~ may already be enough; ~supervisor.el~ is for an
interactive Emacs-first workflow.
...
- *Interactive control plane first*: ~M-x supervisor~ is the core experience:
  a live dashboard for lifecycle actions, policy changes, dependency
  inspection, logs, and status in one Emacs buffer.

- *Systemd-style service model, in Elisp*: Unit-style definitions with
  ~:after~, ~:requires~, explicit stages, ~simple~/~oneshot~, and restart
  policies (~always~, ~on-success~, ~on-failure~, ~no~).

- *Persistent runtime policy overrides*: Enable/disable, mask/unmask,
  restart-policy, and logging overrides are available from dashboard and CLI
  and persist across restarts.

- *Robust lifecycle behavior*: Crash recovery, crash-loop protection,
  manual-stop restart suppression, and reconcile-based convergence keep service
  state predictable.

- *Authority-tier unit files*: Vendor/base defaults can be cleanly overridden by
  higher-precedence user tiers, with daemon-reload support.

- *CLI parity for remote/automation workflows*: ~sbin/supervisorctl~ provides
  scriptable control with human and JSON output for terminal and SSH usage.

- *Timer subsystem (experimental)*: Cron-like calendar timers and relative
  timers for oneshot services, including persistent catch-up semantics.

- *Disk-safe logging pipeline*: External log writers, bounded file growth,
  rotate/prune maintenance tooling, and dashboard/CLI log access.
...
#+begin_example
0 3 * * * /path/to/supervisor-logrotate --log-dir ~/.local/state/supervisor/log --signal-reopen
#+end_example
...
is already running, the script exits 0 silently without taking action.
...
the rotated-file naming pattern is only deleted when its parent active
log exists in the directory (e.g. ~log-svc.log~ must exist for
~log-svc.20250101-120000.log~ to be considered a rotated child of
service ~svc~).  Three layers of protection ensure safety:

1. *Parent-exists guard* --- the parent active log must exist in the
   directory.  Without it, the file could be an active log for a
   timestamp-like service ID and is preserved.  This guard requires no
   external tools and is always active.
2. *Children guard* --- once the log has been rotated at least once, the
   rotated child confirms the parent as an active file.
3. *Open-file guard* --- when ~fuser~ is available, files currently held
   open by a process (e.g. the logd writer) are never deleted.

Orphaned rotated files whose parent has been removed are conservatively
preserved by the parent-exists guard; the complementary age-based
pruning in ~supervisor-logrotate --keep-days~ cleans them up over time.
The ~--protect-id~ flag provides additional explicit protection.

A typical cron entry for periodic pruning (e.g. hourly):

#+begin_example
0 * * * * /path/to/supervisor-log-prune --log-dir ~/.local/state/supervisor/log --max-total-bytes 1073741824
#+end_example
...
| ~supervisor-logd-pid-directory~ | ~nil~ (falls back to ~supervisor-log-directory~) | Writer PID file directory |
