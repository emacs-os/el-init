...
- anchored on ~:last-success-at~ uniformly for all target types (oneshot,
  simple, target)
...
- Converging target unit -> skipped, miss reason ~overlap~
...
- ~:last-result-reason~ (type-specific reason symbol, e.g. ~exit-ok~,
  ~spawn-failed~, ~target-degraded~, ~already-active~)
...
column layout: ~ID~, ~TARGET~, ~ENABLED~, ~TYPE~, ~LAST-RUN~, ~NEXT-RUN~,
~EXIT~, ~RESULT~, ~MISS~.  This matches the columns of ~supervisorctl list-timers~.
...
~── Services  TYPE  STAGE  ENABLED  STATUS  RESTART  LOG  PID  REASON~.
Service rows follow under their stage separators.
...
- ~RESULT~: last execution result (~success~, ~failure~, or ~-~)
- ~MISS~: last miss reason (or ~-~)
...
| ~WantedBy=multi-user.target~ | ~:stage stage3~ + ~:enabled t~ | Stage model approximation in current implementation |
