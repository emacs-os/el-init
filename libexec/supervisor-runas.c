...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--user") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "supervisor-runas: --user requires an argument\n");
                return EXIT_USAGE;
            }
            user_arg = argv[++i];
        } else if (strcmp(argv[i], "--group") == 0) {
            if (i + 1 >= argc) {
                fprintf(stderr, "supervisor-runas: --group requires an argument\n");
                return EXIT_USAGE;
            }
            group_arg = argv[++i];
        } else {
            fprintf(stderr, "supervisor-runas: unknown option: %s\n", argv[i]);
            return EXIT_USAGE;
        }
    }
    ...
    if (cmd_start < 0 || cmd_start >= argc) {
        fprintf(stderr,
                ...
                "usage: supervisor-runas --user USER [--group GROUP] -- CMD [ARGS...]\n");
        return EXIT_USAGE;
    }

    if (!user_arg && !group_arg) {
        fprintf(stderr, "supervisor-runas: at least one of --user or --group required\n");
        return EXIT_USAGE;
    }
    ...
    if (user_arg) {
        if (is_numeric(user_arg)) {
            ...
            if (!group_arg) {
                if (!pw) {
                    fprintf(stderr,
                            "supervisor-runas: uid %s exists but has no passwd entry "
                            "(cannot determine primary group)\n", user_arg);
                    return EXIT_RESOLVE;
                }
            ...
            if (!pw) {
                fprintf(stderr, "supervisor-runas: unknown user: %s\n", user_arg);
                return EXIT_RESOLVE;
            }
    ...
    if (group_arg) {
        if (is_numeric(group_arg)) {
            ...
            if (!gr) {
                fprintf(stderr, "supervisor-runas: unknown group: %s\n", group_arg);
                return EXIT_RESOLVE;
            }
    ...
    if (user_name) {
        if (initgroups(user_name, target_gid) != 0) {
            fprintf(stderr, "supervisor-runas: initgroups(%s, %u): %s\n",
                    user_name, (unsigned)target_gid, strerror(errno));
