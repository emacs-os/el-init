...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--file") == 0) {
            if (++i >= argc) {
                fprintf(stderr, "supervisor-logd: --file requires an argument\n");
                return EXIT_USAGE;
            }
            file_path = argv[i];
            ...
            if (++i >= argc) {
                fprintf(stderr,
                        "supervisor-logd: --max-file-size-bytes requires an argument\n");
                return EXIT_USAGE;
            }
            max_size = atoll(argv[i]);
            if (max_size <= 0) {
                fprintf(stderr,
                        "supervisor-logd: --max-file-size-bytes must be positive\n");
                return EXIT_USAGE;
            }
        } else if (strcmp(argv[i], "--log-dir") == 0) {
            if (++i >= argc) {
                fprintf(stderr, "supervisor-logd: --log-dir requires an argument\n");
                return EXIT_USAGE;
            }
            log_dir = argv[i];
        } else if (strcmp(argv[i], "--prune-cmd") == 0) {
            if (++i >= argc) {
                fprintf(stderr, "supervisor-logd: --prune-cmd requires an argument\n");
                return EXIT_USAGE;
            }
            prune_cmd = argv[i];
            ...
            if (++i >= argc) {
                fprintf(stderr,
                        "supervisor-logd: --prune-min-interval-sec requires an argument\n");
                return EXIT_USAGE;
            }
    ...
    if (max_size <= 0) {
        fprintf(stderr, "supervisor-logd: --max-file-size-bytes is required\n");
        usage();
        return EXIT_USAGE;
    }
    ...
    while (!got_term) {
        ...
        if (got_hup) {
            ...
            if (fd < 0) {
                fprintf(stderr, "supervisor-logd: reopen %s: %s\n",
                        file_path, strerror(errno));
                return EXIT_IO;
            }
        ...
        if (n < 0) {
            if (errno == EINTR)
                continue;  /* signal interrupted read; re-check flags */
            /* Real read error */
            break;
        }
        if (n == 0)
        ...
        /* Write to log file */
        {
            ...
            while (written < n) {
                ...
                if (w < 0) {
                    if (errno == EINTR)
                        continue;
                    fprintf(stderr, "supervisor-logd: write %s: %s\n",
                            file_path, strerror(errno));
                    close(fd);
                    return EXIT_IO;
                }
